...
- [~] **M4 (čiastočne)** — Audit log (JSONB), warns (`/warn`, `/warns`, `/mywarns`, `/removewarn`, `/clearwarns` + auto-punishment na threshold), `/find`, `/manualverify`, cleanup pri odchode zo servera, bot presence/avatar. Tickety, scheduler a XML export vynechané (XML zámerne — pozri diskusiu v histórii, nechceli sme umelo prilepenú funkciu).
