...
- [ ] `/code` má teraz Settings modal (predtým nemal žiadny) — textarea "Success message" s placeholdermi `{user}` `{server}` `{ais_id}`; prázdne pole = použije sa default "Úspešne overené! Vitaj.". Pre kanály: tlačidlo/dropdown "+ Insert channel" nad textarea vloží na pozíciu kurzora token `{channel=<id>}` — dá sa vložiť ľubovoľný počet rôznych kanálov na ľubovoľné miesta v texte (nie je to kanál, kde bol príkaz spustený). Ulož správu s viacerými `{channel=...}` tokenmi (napr. kanál na výber rolí + pravidlá), spusti `/verify` + `/code` v Discorde a over: meno, názov servera, AIS ID, a že KAŽDÝ token je samostatný klikateľný odkaz na svoj vlastný kanál (klik ťa naň prenesie).
